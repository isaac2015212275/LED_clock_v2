...
bool canAdjMin, canAdjHour, dotisOn; 
...
void loop() {
  ...
  if(digitalRead(4)==HIGH&&canAdjMin)
    ...
    addTime(true);
  millisNow = millis();
  if(millisNow - minCounterPrev >= 300)
    canAdjMin = true;
  if(millisNow - hourCounterPrev >= 300)
    canAdjHour = true;
  if(millisNow-millisPrev>=1000)
    tickTime();
  else if(millisNow-millisPrev>=600)
    dotisOn = true;
  setTime();
  delay(2);
}
